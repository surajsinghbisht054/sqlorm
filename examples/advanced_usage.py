...
cheap_or_out_of_stock = Product.objects.filter(
    Q(price__lt=50) | Q(quantity_in_stock=0)
)
...
complex_query = Product.objects.filter(
    (Q(price__gte=30) & Q(price__lte=100)) |
    Q(sku__startswith='ELEC')
...
products = execute_raw_sql("SELECT name, price FROM sqlorm_models_product WHERE price > %s", [100])
