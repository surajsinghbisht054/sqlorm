...
class TestConnection:
    ...
    def test_execute_raw_sql(self):
        ...
        execute_raw_sql(
            "CREATE TABLE test_raw (id INTEGER PRIMARY KEY, name TEXT)", fetch=False
        )

        # Insert data
        execute_raw_sql(
            "INSERT INTO test_raw (name) VALUES (?)", ["Test Name"], fetch=False
        )
    ...
    def test_get_table_names(self):
        ...
        # Create a test table
        execute_raw_sql(
            "CREATE TABLE test_tables (id INTEGER PRIMARY KEY)", fetch=False
        )
...
class TestExceptions:
    ...
    def test_exception_catching(self):
        ...
        from sqlorm.exceptions import ConfigurationError, SQLORMError
...
class TestIntegration:
    ...
    def test_complete_workflow(self):
        ...
        # Create data
        author = Author.objects.create(name="Jane Author", email="jane@example.com")

        book1 = Book.objects.create(
            title="First Book", isbn="1234567890123", published=True
        )

        book2 = Book.objects.create(
            title="Second Book", isbn="1234567890124", published=False
        )
    ...
    def test_queryset_chaining(self):
        ...
        # Create sample data
        for i in range(10):
            Task.objects.create(title=f"Task {i}", priority=i % 5, completed=i % 2 == 0)
...
class TestAdvancedQueries:
    ...
    def test_q_objects_or(self):
        ...
        from sqlorm import Model, Q, fields
    ...
    def test_f_expressions(self):
        ...
        from sqlorm import F, Model, fields
    ...
    def test_aggregations(self):
        ...
        from sqlorm import Avg, Count, Max, Min, Model, Sum, fields
        ...
        result = Sale.objects.aggregate(
            max_qty=Max("quantity"), min_qty=Min("quantity")
        )
    ...
    def test_annotations(self):
        ...
        invoices = Invoice.objects.annotate(total=F("unit_price") * F("quantity"))
    ...
    def test_bulk_operations(self):
        ...
        # Bulk create
        items = [BulkItem.objects.create(name=f"Item{i}", value=i) for i in range(5)]
    ...
    def test_ordering_and_slicing(self):
        ...
        # Order ascending
        asc = list(
            OrderedItem.objects.order_by("position").values_list("position", flat=True)
        )
        ...
        # Offset slicing
        middle = list(
            OrderedItem.objects.order_by("position")[3:6].values_list(
                "position", flat=True
            )
        )
        assert middle == [3, 4, 5]
...
class TestRawSQL:
    ...
    def test_raw_sql_select(self):
        ...
        # Insert data
        execute_raw_sql(
            "INSERT INTO test_raw_select (name, value) VALUES (%s, %s)",
            ["A", 10],
            fetch=False,
        )
        execute_raw_sql(
        ...
        results = execute_raw_sql(
            "SELECT name, value FROM test_raw_select ORDER BY value"
        )
    ...
    def test_raw_sql_dict(self):
        ...
        execute_raw_sql(
            "INSERT INTO test_raw_dict (name) VALUES (%s)", ["Test"], fetch=False
        )
...
class TestModelInstanceMethods:
    ...
    def test_queryset_chaining_with_custom_methods(self):
        ...
        items = (
            ChainTestModel.objects.filter(active=True)
            .exclude(category="B")
            .order_by("name")
        )
...
class TestEnhancedExceptions:
    ...
    def test_exception_details(self):
        ...
        config_error = ConfigurationError(
            "Missing config", missing_keys=["ENGINE", "NAME"]
        )
    ...
    def test_exception_hint(self):
        ...
        error = SQLORMError("Connection failed", hint="Check your database credentials")
    ...
    def test_exception_to_dict(self):
        ...
        error = MigrationError(
            "Table creation failed", table_name="users", operation="CREATE TABLE"
        )
