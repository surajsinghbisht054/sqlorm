...
class TestMultiDatabaseConfiguration:
    ...
    def test_missing_default_raises_error(self):
        ...
        with pytest.raises(
            ConfigurationError, match="'default' database configuration is required"
        ):
            configure_databases(
                {
                    "other": {
    ...
    def test_cross_database_queries(self):
        ...
        from sqlorm import Count, Model, Sum, fields
        ...
        for cid in [1, 2]:
            ...
            stats = orders.aggregate(count=Count("id"), total=Sum("total"))
            MDCustomerStats.objects.create(
                customer_id=cid, order_count=stats["count"], total_spent=stats["total"]
            )
    ...
    def test_model_with_foreign_key_same_database(self):
        ...
        class MDAItem(Model):
            ...
            category = fields.ForeignKey(
                MDACategory._django_model, on_delete=fields.CASCADE, null=True
            )
